...
void ANO_DT_Init(void)
{
	...
	/*  step1 添加用户帧初始化 F1 ~ FA 10*/
	dt.fun[0xF1].D_Addr = 0xff;
	dt.fun[0xF1].fre_ms = 20;	  //
...
//	case 0xF1: //step 2 添加发送数据
//	{

		//send_buffer[(*_cnt)++] = 0x01; //u8
...
//   	send_buffer[(*_cnt)++] = visual_sense_tx;
//			send_buffer[(*_cnt)++] = wanc ;
		
		
//     send_buffer[(*_cnt)++] = t265_x ;
//		  send_buffer[(*_cnt)++] = t265_y ;
//			 send_buffer[(*_cnt)++] = visual_sense_tx ;
//	}
//	break;
	default:
